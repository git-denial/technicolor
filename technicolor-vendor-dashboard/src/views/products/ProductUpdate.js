...
const ProductCreate = ({match}) => {
  ...
  const getProduct = async () => {
    try {
      ...
      setSizeDescription(result.size_description)
      let frontImageArr = [];
  ...
  const submission = async () => {
    ...
    try {
      const productModel = new Product();
      let result = await productModel.edit(match.params.id, submit);
      console.log(result);
      alert('Edit Success!')
      return history.push("/my-product/" + vendorId)
    } catch (e) {
      ...
      <CRow>
        <CCol xs="12" md="12">
          <CCard>
            <CCardHeader>
              Edit Product <br/>
              <Link to={"/my-product/" + vendorId}><small>Back to product list</small></Link>
            </CCardHeader>
            <CCardBody>
              <CForm
                action=""
                    ...
                    <FileUpload
                      ...
                      onDrop={async (file) => {
                        try {
                          ...
                          let result = await productModel.uploadImage(match.params.id, formData)
            ...
            <CCardFooter>
              <CRow>
                ...
                >
                  <CIcon name="cil-save"/> Reset changes
                </CButton>
              </CRow>
