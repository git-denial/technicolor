...
const VendorUpdate = ({match}) => {
  ...
  const submission = async () => {
    ...
    try {
      let result = await vendorModel.edit(match.params.id, submit);
      ...
      let resultCategory = await categoryModel.editCategoryVendor(match.params.id, bodyCategory);
      console.log(result);
  ...
  const getVendor = async () => {
    try{
      let result = await vendorModel.getById(match.params.id);
      console.log(result);

      setCategory(result.categories[0].id)
      setOldCategory(result.categories[0].id)
      setEmail(result.email);
      setName(result.name);
      setPhoneNumber(result.phone_num);
      setLogo(result.logo_url);
      setCity(result.city_code);
